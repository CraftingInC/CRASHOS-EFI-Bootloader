...
TEXTPOSITION* textPos __attribute__ ((section(".text")));
...
void itoa64(uint64_t n, uint8_t* buffer, uint32_t basenumber);
...
void main(BLOCKINFO* bi)
{
    ...
    // 256 Megabytes = 268435456 Bytes
	// 512 Megabytes = 536870912 Bytes
	//   1 Gig       = 1073741824 Bytes
	//   4 Gig       = 4294967296 Bytes

    uint64_t TotalRam = 0;
    uint64_t MMapEntries = biStruct->MMapSize / biStruct->MMapDescriptorSize;
	
	for(uint32_t i = 0; i < MMapEntries - 1; i++)
	{
		EFI_MEMORY_DESCRIPTOR* desc = (EFI_MEMORY_DESCRIPTOR*)((uint64_t)biStruct->MMap + (i * biStruct->MMapDescriptorSize));
		TotalRam += desc->NumberOfPages * 4096;
	}

	uint8_t st2[] = "TOTAL RAM :";
	Print(st2, textPos->X, textPos->Y, 2, YELLOW);
	...
	uint8_t st3[] = "Bytes";
	Print(st3, textPos->X, textPos->Y, 2, GRAY);
	
	textPos->X  = 20;
	textPos->Y += 32;

	uint8_t st4[] = "RSDP      :";
	Print(st4, textPos->X, textPos->Y, 2, YELLOW);

    textPos->X += 180;

	uint8_t st6[] = "0x";
	Print(st6, textPos->X, textPos->Y, 2, WHITE);
	...
	uint8_t st5[] = "End Program";
	Print(st5, textPos->X, textPos->Y, 1, GREEN);
...
void ClearScreen(unsigned int c)
{
	unsigned int x = 0;
	unsigned int y = 0;
	unsigned int w = (biStruct->ScreenWidth)  / 4;
	unsigned int h = (biStruct->ScreenHeight) / 4;
	CreateFilledBox(x, y, w, h, c);
}
	...
	for(unsigned long fc = character; fc < (character + 4); fc++)
	{
		for(long t = BITS32; t >= 0; t--)
		{
			if(togglearray == 0)
			{
				...
				{
					CreateFilledBox(x, y, FontSize, FontSize, c);
				}
